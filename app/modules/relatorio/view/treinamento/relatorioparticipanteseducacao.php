<table>
	...
	<td>
	    <h1><?php echo $this->t( 'Participantes por por nível de educação', 368 ); ?> </h1>
	</td>
	<td style="width:1%">
...
<table id="relatorio">
    <thead>
	<tr>
	    <th><?php echo $this->t( 'Nível de Educação', 402 ); ?></th>
...
<script type="text/javascript">
	...
	$( document ).ready(
		...
		chartPie = new Highcharts.Chart({
		    ...
		    title: {
			text: '<?php echo $this->t( 'Participantes por por nível de educação', 368 ); ?> (%)'
		    },
		    tooltip: {
		...
		chartBar = new Highcharts.Chart({
		    ...
		    title: {
			text: '<?php echo $this->t( 'Participantes Homem x Mulher para cada nível', 403 ); ?> (%)'
		    },
		    xAxis: {
			categories: <?php echo json_encode( $this->data['graph_bar']['educacao'] ); ?>,
			title: {
			    text: null
			}
		    },
